    ...
    <>
      ...
      {open && (
        <div className="navbar__drawer">
          {NAV_LINKS.map((l, i) => (
            <button key={l.label}
              className={`navbar__drawer-btn${active === l.href.slice(1) ? ' navbar__drawer-btn--active' : ''}`}
              onClick={() => handleNav(l.href)}
              style={{ animation: `fadeUp .3s ease ${i * .06}s both` }}>
              <span className="navbar__drawer-num">0{i}</span>
              {l.label}
            </button>
          ))}
          <a href="mailto:leoroshan2002@gmail.com" className="navbar__drawer-cta">
            Get In Touch
          </a>
        </div>
      )}
...
function NavItem({ link, isActive, onNav }) {
  return (
    <li>
      <button
        className={`navbar__link-btn${isActive ? ' navbar__link-btn--active' : ''}`}
